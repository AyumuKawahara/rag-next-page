...
> **表の読み方:** 行は「LLMが実際に選択した行動」、列は「反実仮想評価で求めた根拠取得上の最良行動集合」です。各セルは、その組合せに該当した探索状態数を表します。

LLMの選択が列の最良行動集合に含まれるセルを太字で示します。例えば、`再検索・追加取得`列では、再検索と下位検索結果の追加取得の両方が根拠取得上の最良行動です。

## 論文掲載条件（k=3）

### IIRC

| 選択モデル | LLMが実際に選択した行動<br>（行） | 最良行動集合：回答<br>（列） | 最良行動集合：再検索<br>（列） | 最良行動集合：追加取得<br>（列） | 最良行動集合：再検索・追加取得<br>（列） |
|---:|---|---:|---:|---:|---:|
| 4B | 回答 | **287** | 123 | 24 | 35 |
|  | 再検索 | 92 | **121** | 14 | **18** |
|  | 追加取得 | 54 | 57 | **8** | **13** |
